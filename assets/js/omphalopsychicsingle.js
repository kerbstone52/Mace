...
function compute() {
    ...
    jd = 367 * agea - Math.floor(7 * (agea + 5001 + Math.floor((month - 9) / 7)) / 4) + Math.floor(275 * month / 9) + day + 1729776.5 + ut / 24
    //epoch 2000.0
    T = (jd - 2451545) / 36525
    //epoch 1900.5    
    tt = (agea - 1900) / 100
    ...
    obliquity = (23.4392911 - 0.130025833 * Tbret - 0.00000430556 * Math.pow(Tbret, 2) + 0.000555347 * Math.pow(Tbret, 3) - 0.00000142722 * Math.pow(Tbret, 4) - 0.000000693528 * Math.pow(Tbret, 5) - 0.0000000108472 * Math.pow(Tbret, 6) + 0.000000000197778 * Math.pow(Tbret, 7)) / 180 * Math.PI
    ...
    tropicalyear = 365.2421896698 - 0.00000615359 * T - 0.000000000729 * T * T + 0.000000000264 * T * T * T
    eclipa = obliquity
    ...
    if (methoda == 1) {
        ...
        calcappalt = 0.057288 * (ha - hha) / kma - 0.00447387 * kma + 0.008296359 * ka * kma * pa / ta / ta
        // determine the lowest altitude due to zero level sea
        checkappalt = -0.03203 * Math.sqrt(hha) * Math.sqrt(1 - 1.848042142 * ka * pa / ta / ta)
        if (checkappalt > calcappalt) {

            alert("WARNING: The vast plain horizon at sea level is above the DISTANT OBJECT HEIGHT");
    ...
    if (methoda == 2) {
        ...
        calcappalt = -0.03203 * Math.sqrt(hha - ha) * Math.sqrt(1 - 1.848042142 * ka * pa / ta / ta)
    }
    if (methoda == 3) {
        givenappalt = AA
        if (givenappalt < -2 || isNaN(givenappalt) || givenappalt > 30) {
            alert("ERROR: APPARENT ALTITUDE should be higher than -2° and smaller than 30°")
        }
    ...
    if (methoda == 4) {
        ...
        calcalt = calcappalt - pa * (0.1594 + 0.0196 * calcappalt + 0.00002 * calcappalt * calcappalt) / ta / (1 + 0.505 * calcappalt + 0.0845 * calcappalt * calcappalt)

        erroralta = errorappalta * (1 - pa / ta / (1 + 0.505 * calcappalt + 0.0845 * calcappalt * calcappalt) / (1 + 0.505 * calcappalt + 0.0845 * calcappalt * calcappalt) * ((1 + 0.505 * calcappalt + 0.0845 * calcappalt * calcappalt) * (0.0196 + 0.00004 * calcappalt) - (0.1594 + 0.0196 * calcappalt + 0.00002 * calcappalt * calcappalt) * (0.505 + 0.169 * calcappalt)));
    }
    ...
    //average parallax
    if (objecta == 1) {
        par = 0.952 / 180 * Math.PI * (Math.cos(calcalt) - Math.sin(calcalt * 2) * 0.008258773)
    }
    if (objecta == 0) {
    ...
    decla = Math.asin((Math.sin(lata) * Math.sin(calcalt + par) + Math.cos(lata) * Math.cos(calcalt + par) * Math.cos(azia)))
    decl = Math.round(180 / Math.PI * decla * 100) / 100
    alta = calcalt;
    // determing the error in declanation: http://www.iol.ie/~geniet/eng/accuracy.htm
    errordazi = errorazia / Math.cos(decla) * (Math.sin(lata) * Math.sin(alta) - Math.cos(lata) * Math.cos(alta) * Math.sin(azia));
    errordalt = erroralta / Math.cos(decla) * (Math.sin(lata) * Math.cos(alta) - Math.cos(lata) * Math.sin(alta) * Math.cos(azia));
    errordlat = errorlata / Math.cos(decla) * (Math.cos(lata) * Math.sin(alta) - Math.sin(lata) * Math.cos(alta) * Math.cos(azia));
    errordecl = Math.sqrt(errordazi * errordazi + errordalt * errordalt + errordlat * errordlat)
    errordecl = Math.round(errordecl * 100) / 100;
    riseangle = Math.round(180 / Math.PI * Math.acos(Math.sin(lata) / Math.cos(decla)))
    ...
    if (objecta == 0) {
        ...
        errordagen = Math.floor(0.5 + Math.abs(-errordecl / 180 * Math.PI / Math.sqrt(1 - Math.sin(decla) / Math.sin(eclipa) * Math.sin(decla) / Math.sin(eclipa)) * Math.cos(decla) / Math.sin(eclipa) / .9856 / Math.PI * 180))
        datum = datumzon.getDate() + " - " + (datumzon.getMonth() + 1) + " - " + datumzon.getFullYear();
        datum2 = datumzon2.getDate() + " - " + (datumzon2.getMonth() + 1) + " - " + datumzon2.getFullYear();
    } else {
        dagen = "NaN";
        errordagen = "NaN";
        datum = "NaN";
    ...
    azimuth = 180 / Math.PI * Math.acos((Math.sin(hoek) - Math.sin(lata) * Math.sin(calcalt)) / Math.cos(lata) / Math.cos(calcalt))
    ...
    azimuthc = 180 / Math.PI * Math.acos((Math.sin(hoek3) - Math.sin(lata) * Math.sin(calcalt)) / Math.cos(lata) / Math.cos(calcalt))
    ...
    azimuthb = 180 / Math.PI * Math.acos((Math.sin(hoek2) - Math.sin(lata) * Math.sin(calcalt)) / Math.cos(lata) / Math.cos(calcalt))
    ...
    if (soad == 1) {
        ...
        calcalta = calcalt + 1.004 / 180 * Math.PI * (Math.cos(calcalt) - Math.sin(calcalt * 2) * 0.008258773)
    }

    sooud = soad


    azimuthd = 180 / Math.PI * Math.acos((Math.sin(hoek4) - Math.sin(lata) * Math.sin(calcalta)) / Math.cos(lata) / Math.cos(calcalta))
    ...
    if (soae == 1) {
        ...
        calcaltb = calcalt + .900 / 180 * Math.PI * (Math.cos(calcalta) - Math.sin(calcalta * 2) * 0.008258773)
    }
    //alert(soa)
    sooud = soae


    azimuthe = 180 / Math.PI * Math.acos((Math.sin(hoek5) - Math.sin(lata) * Math.sin(calcaltb)) / Math.cos(lata) / Math.cos(calcaltb))
...
function perihelionday(angle, tweede) {
    ...
    dayoud = Math.acos(Math.sin(angle / 180 * Math.PI) / Math.sin(obliquity)) / Math.PI * 180 / 0.9856
    if (tweede == 1) {
        dayoud = -dayoud
    }
    //if (stap==0) {
...
function declination(day) {
    angle = Math.asin(Math.sin(obliquity) * Math.cos((360 / tropicalyear * day + eccentricityvalue * 2 * 180 / Math.PI * Math.sin(360 / tropicalyear * (day - periheliondaynumber) / 180 * Math.PI)) / 180 * Math.PI)) / Math.PI * 180
    return angle
}
